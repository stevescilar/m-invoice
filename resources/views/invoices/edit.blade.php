...
<form method="POST" action="{{ route('invoices.update', $invoice) }}" x-data="invoiceEditForm()" class="space-y-6">
@csrf @method('PUT')
            ...
            <div class="space-y-3">
                ...
                <div>
                    ...
                    <textarea name="notes" rows="2"
                        class="w-full border rounded-lg px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-400">{{ $invoice->notes }}</textarea>
                </div>
            </div>
        </div>
            ...
            <div class="space-y-3">
                ...
                <div x-show="isRecurring" x-cloak>
                    ...
                    <select name="recurrence_interval"
                        class="w-full border rounded-lg px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-400">
                        <option value="monthly" {{ $invoice->recurrence_interval === 'monthly' ? 'selected' : '' }}>Monthly</option>
                        <option value="weekly" {{ $invoice->recurrence_interval === 'weekly' ? 'selected' : '' }}>Weekly</option>
                    </select>
                </div>
            </div>
        </div>
...
function invoiceEditForm() {
    return {
        ...
        isRecurring: {{ $invoice->is_recurring ? 'true' : 'false' }},
        materialCost: {{ $invoice->material_cost }},
        labourCost: {{ $invoice->labour_cost }},
