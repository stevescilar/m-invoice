...
<div class="flex items-center gap-3 mb-6">
    <a href="{{ route('invoices.show', $invoice) }}" class="text-gray-400 hover:text-gray-600">← Back</a>
    <div>
        <h1 class="text-2xl font-bold text-gray-800">Edit Invoice</h1>
        <p class="text-xs text-gray-400 mt-0.5">{{ $invoice->invoice_number }} · {{ now()->format('l, d M Y • H:i') }}</p>
    </div>
</div>
...
{{--
    ...
    Integer vs string comparison causes the dropdown to always fall back to the first option (Material).
--}}
<script>
    ...
    const existingItems = @json(collect($invoiceItems)->map(fn($i) => array_merge($i, ['item_type_id' => (string)($i['item_type_id'] ?? '')])));
</script>

<form method="POST" action="{{ route('invoices.update', $invoice) }}" x-data="invoiceForm()" class="space-y-6">
    @csrf @method('PUT')
                ...
                <div class="space-y-3">
                    ...
                    <div>
                        ...
                        <textarea name="notes" rows="2"
                            class="w-full border rounded-lg px-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-green-400">{{ old('notes', $invoice->notes) }}</textarea>
                    </div>
                </div>
            </div>
...
function invoiceForm() {
    return {
        ...
        etrEnabled:        {{ $invoice->etr_enabled ? 'true' : 'false' }},
        typeBreakdown:     {},
        typeBreakdownList: [],
        vatAmount:         0,
        grandTotal:        0,
